...
struct DiffuseBSDF : BSDF {
    ...
    v3f eval(const SurfaceInteraction& i) const override {
        ...
        if(i.frameNs.cosTheta(i.wi) > 0 && i.frameNs.cosTheta(i.wo) > 0){
            val = albedo->eval(worldData, i) / M_PI * i.frameNs.cosTheta(i.wi);
        }
    ...
    v3f sample(SurfaceInteraction& i, const v2f& sample, float* pdf) const override {
        ...
        glm::vec3 sampleDir = glm::normalize(Warp::squareToCosineHemisphere(sample));
        i.wi = sampleDir;

        *pdf = DiffuseBSDF::pdf(i);

        if(*pdf != 0)
            val = DiffuseBSDF::eval(i) / *pdf;
